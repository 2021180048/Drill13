...
class Zombie:
    ...
    def set_random_location(self):
        self.tx, self.ty = random.randint(100, 1280-100), random.randint(100, 700-100)
        return BehaviorTree.SUCCESS
        pass
    ...
    def build_behavior_tree(self):
        ...
        c2 = Condition('좀비 공이 많은가?', self.boy_ball_count_win)

        SEQ_move_to_target_location = Sequence('Move to target lacation', a1, a2)
        SEQ_patrol = Sequence('순찰', a5, a2)
        SEQ_run_away = Sequence('소년으로 부터 도망', c2, a6)
        SEQ_chase_boy = Sequence('소년을 추적', a4)
        SEQ_wander = Sequence('Wander', a3, a2)
        SEL_chase_or_run_away = Selector('도망 또는 추적', SEQ_run_away, SEQ_chase_boy)
        SEQ_checking_conditions = Sequence('조건 확인', c1, SEL_chase_or_run_away)
        SEL_checking_or_wander = Selector('확인 또는 배회', SEQ_checking_conditions, SEQ_wander)

        root = SEL_checking_or_wander
